...
class RealESRNet(object):
    ...
    def load_srmodel(self, base_dir, model):
        ...
        if model is None:
            loadnet = torch.load(os.path.join(self.base_dir, 'models', "face_enhance_model", 'realesrnet_x%d.pth'%self.scale))
        else:
            loadnet = torch.load(os.path.join(self.base_dir, 'models', "face_enhance_model", model+'_x%d.pth'%self.scale))
